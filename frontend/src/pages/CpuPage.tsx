...
function getStatusColor(util: number, temp: number): { color: string; label: string } {
  if (temp > 90 || util > 95) return { color: resolveVar('--danger'), label: 'Critical' };
  if (temp > 75 || util > 80) return { color: resolveVar('--warning'), label: 'Warning' };
  return { color: resolveVar('--success'), label: 'Normal' };
}
...
function CpuVerticalProgress({ value, label, type = 'percent', max = 100 }: {
  value: number;
  label: string;
  type?: 'percent' | 'temp';
  max?: number;
  ...
  if (type === 'temp') {
    ...
    displayValue = type === 'percent' ? `${Math.round(value)}%` : value.toFixed(2);
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, minHeight: 0, gap: 4 }}>
      ...
      <div style={{ fontSize: 9, color: resolveVar('--text-muted'), textTransform: 'uppercase', letterSpacing: 1.2, flexShrink: 0, whiteSpace: 'nowrap' }}>{label}</div>
    </div>
  );
}
...
function CpuSummaryCard({ accent }: { accent: { color: string; glow: string } }) {
    ...
    <div className="metric-card" style={{ display: 'flex', flexDirection: 'column', flexShrink: 0, flex: 1, minHeight: 0 }}>
      ...
      <div className="card-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span className="card-title" style={{ fontSize: '12px' }}>CPU</span>
        </div>
      ...
      <div className="card-details cpu-metrics-grid" style={{ margin: '0 2px', flexShrink: 0 }}>
        ...
        <div className="card-detail-item">
          <span className="card-detail-label"><Zap size={11} style={{ marginRight: 4, verticalAlign: 'middle' }}/>Freq</span>
          <span className="card-detail-value" style={{ color: accent.color }}>
            {freq > 0 ? `${(freq / 1000).toFixed(1)} GHz` : '—'}
          </span>
        </div>
        <div className="card-detail-item">
          <span className="card-detail-label"><Server size={11} style={{ marginRight: 4, verticalAlign: 'middle' }}/>Cores</span>
          <span className="card-detail-value" style={{ color: accent.color }}>
            {physCores}
          </span>
        </div>
        <div className="card-detail-item">
          <span className="card-detail-label"><Activity size={11} style={{ marginRight: 4, verticalAlign: 'middle' }}/>Threads</span>
          <span className="card-detail-value" style={{ color: accent.color }}>
            {threads}
          </span>
        </div>
        <div className="card-detail-item">
          <span className="card-detail-label"><Activity size={11} style={{ marginRight: 4, verticalAlign: 'middle' }}/>Load 1m</span>
          <span className="card-detail-value" style={{ color: accent.color }}>
            {load1.toFixed(2)}
          </span>
        </div>
        <div className="card-detail-item">
          <span className="card-detail-label"><Activity size={11} style={{ marginRight: 4, verticalAlign: 'middle' }}/>Load 5m</span>
          <span className="card-detail-value" style={{ color: accent.color }}>
            {load5.toFixed(2)}
          </span>
        </div>
        <div className="card-detail-item">
          <span className="card-detail-label"><Activity size={11} style={{ marginRight: 4, verticalAlign: 'middle' }}/>Load 15m</span>
          <span className="card-detail-value" style={{ color: accent.color }}>
            {load15.toFixed(2)}
          </span>
        </div>
      ...
      <div style={{
        ...
        <CpuVerticalProgress value={util} label="CPU UTIL" />
        <CpuVerticalProgress value={load1Pct} label="LOAD 1M" />
        <CpuVerticalProgress value={load5Pct} label="LOAD 5M" />
        <CpuVerticalProgress value={load15Pct} label="LOAD 15M" />
        <CpuVerticalProgress value={temp} label="TEMP" type="temp" max={100} />
      </div>
    </div>
  );
}
...
function CpuContent({ accent }: CpuPageProps) {
  ...
  const coreData = useMemo(() => {
    ...
    return cpuRawData.cores as Array<{ utilization_percent: number; temperature_celsius?: number } | null>;
  }, [cpuRawData?.cores]);

  const hasHistory = cpuHistory && cpuHistory.length > 0;
  const hasTempHistory = cpuTemperatureHistory && cpuTemperatureHistory.length > 0;
  ...
  return (
    <div className="cpu-row">
      ...
      <div className="cpu-charts" style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0 }}>
    ...
    return (
      <main className="dashboard-grid">
        <PanelErrorBoundary panelName="CPU">
          <PanelErrorState panelName="CPU" error={new Error(cpuError)} errorInfo={null} onRetry={retryCpu} />
        </PanelErrorBoundary>
      </main>
    );
  }
