...
describe('extractQuant', () => {
  it('extracts Q3_K_L from end of filename', () => {
    expect(extractQuant('Qwen3.6-35B-A3B-REAM-192-heretic-APEX-ICompact-Q3_K_L')).toBe('Q3_K_L');
  });

  it('extracts Q3_K_M from mid-filename (delimiter before token)', () => {
    expect(extractQuant('Qwen3.6-35B-A3B-UD-Q3_K_M-REAP-RangerX')).toBe('Q3_K_M');
  });
  ...
  it('extracts Q4_K_M', () => {
    expect(extractQuant('Qwen3.5-9B-Uncensored-HauhauCS-Aggressive-Q4_K_M')).toBe('Q4_K_M');
  });
...
describe('calcBuildsBehind', () => {
  ...
  it('returns null when local is null', () => {
    expect(calcBuildsBehind(null, 'b9851')).toBe(null);
  });

  it('returns null when latest is null', () => {
    expect(calcBuildsBehind('b9833', null)).toBe(null);
  });

  it('returns null when both are null', () => {
    expect(calcBuildsBehind(null, null)).toBe(null);
  });
