...
export type SortColumn = 'status' | 'model' | 'params' | 'quant' | 'ctx' | 'vram' | 'ram' | 'spec' | 'tps';
...
const isUnknown = (val: any): boolean => {
  return val === undefined || val === null || val === "" || val === "—" || Number.isNaN(val);
};
  ...
  return [...profiles].sort((a, b) => {
    ...
    switch (column) {
      case 'status':
        ...
        break;
      case 'model':
        valA = (a.parsed_args?.model_path?.split("/").pop()?.replace(/\.gguf$/i, "") ?? a.name).toLowerCase();
        valB = (b.parsed_args?.model_path?.split("/").pop()?.replace(/\.gguf$/i, "") ?? b.name).toLowerCase();
        break;
      case 'params':
      ...
      case 'ctx':
        ...
        break;
      case 'vram':
        valA = states[a.script_path]?.peak_vram_mb ?? metadata[a.script_path]?.peak_vram_mb;
        valB = states[b.script_path]?.peak_vram_mb ?? metadata[b.script_path]?.peak_vram_mb;
        break;
      case 'ram':
        valA = states[a.script_path]?.peak_ram_mb ?? metadata[a.script_path]?.peak_ram_mb;
        valB = states[b.script_path]?.peak_ram_mb ?? metadata[b.script_path]?.peak_ram_mb;
        break;
      case 'spec':
        ...
        break;
      case 'tps':
        const tpsA = states[a.script_path]?.current_tps ?? metadata[a.script_path]?.avg_gen_tps;
        const tpsB = states[b.script_path]?.current_tps ?? metadata[b.script_path]?.avg_gen_tps;
        valA = tpsA ?? NaN;
        valB = tpsB ?? NaN;
        break;
      default:
