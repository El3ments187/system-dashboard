...
vi.mock("recharts", () => ({
  ...
  CartesianGrid: vi.fn(() => (
    <div data-testid="cartesian-grid">CartesianGrid</div>
  )),
...
describe("PerCoreCpuChart", () => {
  ...
  it("assigns a distinct legend color to each active core", async () => {
    ...
    await waitFor(() => {
      expect(
        container.querySelectorAll('[data-testid="per-core-legend-swatch"]')
          .length,
      ).toBe(8);
    });
    const { unique } = collectPerCoreColors(
      container.querySelectorAll('[data-testid="per-core-legend-swatch"]'),
    );
  ...
  it("keeps legend colors stable across re-renders when theme is unchanged", async () => {
    ...
    const before = collectPerCoreColors(
      container.querySelectorAll('[data-testid="per-core-legend-swatch"]'),
    ).colors;
    rerender(
      <PerCoreCpuChart
    ...
    const after = collectPerCoreColors(
      container.querySelectorAll('[data-testid="per-core-legend-swatch"]'),
    ).colors;
