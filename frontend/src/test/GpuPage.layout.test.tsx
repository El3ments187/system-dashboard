...
describe('GpuPage - layout structure', () => {
  ...
  it('renders multiple GPUs when provided', () => {
    const { container } = renderGpuPage(<GpuPage accent={accent} />, {
      gpuRawData: [
        { name: 'GPU A', utilization_percent: 50, temperature_celsius: 60, vram_used_gb: 4.0, vram_total_gb: 8, power_usage_watts: 100, power_limit_watts: 200, fan_speed_rpm: 800, clock_speed_mhz: 1500, memory_clock_mhz: 700 },
        { name: 'GPU B', utilization_percent: 80, temperature_celsius: 85, vram_used_gb: 6.0, vram_total_gb: 12, power_usage_watts: 200, power_limit_watts: 300, fan_speed_rpm: 1500, clock_speed_mhz: 2000, memory_clock_mhz: 900 },
      ],
    });
...
describe('GpuPage - accent application', () => {
  ...
  it('displays temperature with correct unit', () => {
    ...
    expect(screen.getByText((_text, node) =>
      node?.textContent === '72°C' && node?.className === 'card-detail-value'
    )).toBeInTheDocument();
  });
  ...
  it('applies warning state for high utilization', () => {
    renderGpuPage(<GpuPage accent={accent} />, {
      gpuRawData: [{ name: 'Test GPU', utilization_percent: 75, temperature_celsius: 60, vram_used_gb: 4.0, vram_total_gb: 8, power_usage_watts: 100, power_limit_watts: 200, fan_speed_rpm: 800 }],
    });
...
describe('GpuPage - history charts', () => {
  ...
  it('renders VRAM utilization history chart', () => {
    const { container } = renderGpuPage(<GpuPage accent={accent} />);
    expect(container.querySelectorAll('.chart-container').length).toBeGreaterThan(1);
  });
  ...
  it('does not render history charts when no data available', () => {
    const { container } = renderGpuPage(<GpuPage accent={accent} />, {
      gpuHistory: [], gpuVramUtilHistory: [], gpuTemperatureHistory: [],
    });
  ...
  it('applies accent color to chart titles', () => {
    const { container } = renderGpuPage(<GpuPage accent={accent} />);
    expect(container.querySelectorAll('.card-header').length).toBeGreaterThan(0);
  });
});
