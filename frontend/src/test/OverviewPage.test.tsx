...
function createMockFetch(overrides?: { storageDevices?: any[] }) {
  const storageDevices = overrides?.storageDevices ?? [
    {
      device: "/dev/sda",
      io_stats: {
        reads: 150000,
  ...
  return (url: string) => {
        ...
        json: () =>
          Promise.resolve({
            data: {
              ...
              cores: [
                { utilization_percent: 45, temperature_celsius: 62 },
                { utilization_percent: 52, temperature_celsius: 63 },
                { utilization_percent: 38, temperature_celsius: 61 },
                { utilization_percent: 70, temperature_celsius: 65 },
              ],
            },
          }),
      });
    ...
    if (url === "/api/metrics/storage/devices")
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ data: storageDevices }),
      });
    if (url === "/api/metrics/storage/history")
      return Promise.resolve({
        ok: true,
    ...
    if (url.includes("/metrics/system"))
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ cpu_percent: 45, memory_percent: 72.5 }),
      });
    if (url === "/api/alerts")
...
describe("Overview Page - Solid Mode", () => {
  ...
  describe("layout structure", () => {
    it("renders main dashboard grid container", async () => {
      ...
      await waitFor(
        () =>
          expect(
            container.querySelector(".dashboard-grid"),
          ).toBeInTheDocument(),
        { timeout: 4000 },
      );
    });
    it("renders overview GPU row", async () => {
      ...
      await waitFor(
        () =>
          expect(
            container.querySelector(".overview-gpu-row"),
          ).toBeInTheDocument(),
        { timeout: 4000 },
      );
    });
    it("renders overview CPU row", async () => {
      ...
      await waitFor(
        () =>
          expect(
            container.querySelector(".overview-cpu-row"),
          ).toBeInTheDocument(),
        { timeout: 4000 },
      );
    });
    it("renders overview memory row", async () => {
      ...
      await waitFor(
        () =>
          expect(
            container.querySelector(".overview-memory-row"),
          ).toBeInTheDocument(),
        { timeout: 4000 },
      );
    });
    it("renders storage row", async () => {
      ...
      await waitFor(
        () =>
          expect(container.querySelector(".storage-row")).toBeInTheDocument(),
        { timeout: 4000 },
      );
    });
    ...
    it("renders CPU card in overview", async () => {
      ...
      await waitFor(
        () =>
          expect(
            document.querySelector(".overview-cpu-row"),
          ).toBeInTheDocument(),
        { timeout: 4000 },
      );
    });
  ...
  describe("Turquoise accent application", () => {
    it("applies accent color to GPU card metric values", async () => {
      ...
      await waitFor(
        () => {
          const gpuDetailValues =
            container.querySelectorAll(".card-detail-value");
  ...
  describe("edge cases", () => {
    ...
    it("renders all four card types in overview simultaneously", async () => {
      ...
      await waitFor(
        () =>
          expect(
            document.querySelector(".overview-gpu-row"),
          ).toBeInTheDocument(),
        { timeout: 4000 },
      );
      await waitFor(
