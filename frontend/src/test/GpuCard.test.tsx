...
describe('GpuCard', () => {
  ...
  describe('accent color application', () => {
    ...
    it('shows fallback for temperature when null', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, null, 8.5, 12.0, 250, 300] })}>
    ...
    it('shows fallback for VRAM when values are null', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 72, null, null, 250, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('shows fallback for power when values are null', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 72, 8.5, 12.0, null, null] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('shows Warning status when utilization >= 70%', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [75, 72, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('shows Critical status when utilization >= 90%', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [95, 72, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
  ...
  describe('loading state', () => {
    it('shows skeleton loader when loading', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuLoading: true })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
  ...
  describe('error state', () => {
    it('shows error UI when gpuError is set', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuError: 'Connection failed' })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('provides retry button on error', () => {
      ...
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuError: 'Connection failed', retryGpu: mockRetry })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
  ...
  describe('null data handling', () => {
    it('shows fallback for null GPU utilization', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [null, 72, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('shows fallback for all null values', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [null, null, null, null, null, null] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
  ...
  describe('Turquoise accent mode', () => {
    it('renders correctly with Turquoise accent', () => {
      const { container } = renderWithProviders(<GpuCard accent={turquoiseAccent} />);
    ...
    it('shows correct temperature with Turquoise accent', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 85, 8.5, 12.0, 250, 300] })}>
    ...
    it('handles high utilization with Turquoise accent', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [92, 85, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={turquoiseAccent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('renders progress bar with normal state for low utilization', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [30, 72, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={turquoiseAccent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('displays VRAM percentage correctly', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 72, 10.0, 12.0, 250, 300] })}>
            <GpuCard accent={turquoiseAccent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('displays power limit correctly', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 72, 8.5, 12.0, 200, 350] })}>
            <GpuCard accent={turquoiseAccent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
  ...
  describe('theme color leak detection', () => {
    ...
    it('does not apply the danger color to a Normal-state status dot', () => {
      const { container } = renderWithProviders(<GpuCard accent={turquoiseAccent} />); // 65% / 72°C -> Normal
      const statusDot = container.querySelector('.status-dot');
    ...
    it('applies the warning color at the GPU utilization warning threshold (status dot is utilization-driven, not temperature)', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [75, 50, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={turquoiseAccent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('stays Normal-colored even at a high temperature, since the compact card status dot tracks utilization only', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [50, 95, 8.5, 12.0, 250, 300] })}>
            <GpuCard accent={turquoiseAccent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
  ...
  describe('edge cases', () => {
    it('handles zero utilization', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [0, 40, 0, 12.0, 0, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('handles maximum utilization', () => {
      const { container } = render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [100, 95, 12.0, 12.0, 300, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('handles partial VRAM data', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 72, 8.5, null, 250, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
    ...
    it('handles partial power data', () => {
      render(
        <TooltipProvider>
          <MetricsContext.MetricsContext.Provider value={mockMetricsContext({ gpuCurrentValues: [65, 72, 8.5, 12.0, null, 300] })}>
            <GpuCard accent={accent} />
          </MetricsContext.MetricsContext.Provider>
        </TooltipProvider>
      );
