...
function computeStats(history: MetricHistoryPoint[] | null): { current: number | null; avg: number; peak: number } {
  const values = history?.filter(p => p.value != null && p.value > 0).map(p => p.value as number) ?? [];
      ...
      <div ref={chartRef} style={{ height, overflow: 'hidden' }}>
        <div style={{ width: chartSize.width, height: chartSize.height }}>
          <AreaChart data={chartData} width={chartSize.width} height={chartSize.height} margin={{ top: 2, right: 0, left: 0, bottom: 0 }} padding={{ top: 2, right: 2, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="x" type="number" domain={[0, dataMaxX]} tick={{ fontSize: 8, fill: 'var(--text-muted)' }} axisLine={{ stroke: chartColors.axis }} tickFormatter={(v: number) => { const pt = chartData[Math.round(v)]; return pt ? pt.timeLabel : ''; }} interval="equidistantPreserveStart" />
            <YAxis width={24} type="number" domain={[0, 'dataMax']} tick={{ fontSize: 8, fill: 'var(--text-muted)' }} axisLine={{ stroke: chartColors.axis }} />
...
function StatBadge({ label, current, avg, peak, color }: { label: string; current: number | null; avg: number; peak: number; color: string }) {
  return (
