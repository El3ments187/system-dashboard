...
function formatIops(iops: number): string {
  ...
  const i = Math.min(
    Math.floor(Math.log(iops) / Math.log(1000)),
    units.length - 1,
  );
...
function getDeviceState(io: any): string {
  if (!io) return "Idle";
  const totalBps =
    Math.abs(io.read_bytes_per_sec || 0) +
    Math.abs(io.write_bytes_per_sec || 0);
          ...
          {devices.map((device, di) => {
            ...
            return (
              <div
                key={di}
                style={{
                  marginBottom: di === devices.length - 1 ? 0 : 6,
                  borderBottom:
                    di === devices.length - 1
                      ? "none"
              ...
              >
                    ...
                    style={{
                      ...
                      color:
                        device.temperature_celsius != null
                          ? getStorageTempColor(device.temperature_celsius)
                          : "var(--text-muted)",
                      ...
                      tooltip.setCardTooltip(
                        {
                          ...
                          description: `${device.device} temperature in Celsius`,
                          unit: "°C",
                        },
                        e,
                      )
                ...
                >
                  <span
                    ...
                    onMouseEnter={(e) => {
                      const desc = getMetricDescription(
                  ...
                  <span
                    ...
                    onMouseEnter={(e) => {
                      const desc = getMetricDescription("storage_read_iops");
                      if (desc)
                        tooltip.setCardTooltip(
                  ...
                  <span
                    ...
                    onMouseEnter={(e) => {
                      const desc = getMetricDescription("storage_write_iops");
                      if (desc)
                        tooltip.setCardTooltip(
                          { title: desc.title, description: desc.description },
                          e,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  ...
                  >
                    {device.mounts.length} mount
                    {device.mounts.length > 1 ? "s" : ""}
                  </span>
                </div>
                {/* Mount rows */}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        <span
                          style={{
                            fontSize: 10,
                            color: getUtilColor(mount.utilization_percent),
                            fontFamily: "monospace",
                            flexShrink: 0,
                            cursor: "help",
                          }}
                          onMouseEnter={(e) => {
                            ...
                            if (desc)
                              tooltip.setCardTooltip(
                                {
                                  title: `${mount.mount_point} - ${desc.title}`,
                                  description: desc.description,
                                  unit: desc.unit,
                                },
                                e,
                      ...
                      <ProgressBar
                        percent={mount.utilization_percent}
                        variant="compact"
                      />
                    </div>
                    <div
