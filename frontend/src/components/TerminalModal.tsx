  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') { e.preventDefault(); sendInput(); }
      ...
      if (historyIndex > 0) {
        ...
        setHistoryIndex(newIndex);
        setInputValue(commandHistory[commandHistory.length - 1 - newIndex] || '');
      } else { setHistoryIndex(-1); setInputValue(''); }
      ...
      <div onClick={e => e.stopPropagation()} style={{ flex: 1, display: 'flex', flexDirection: 'column', margin: 8, borderRadius: 6, overflow: 'hidden', background: '#1a1a2e',         border: '1px solid var(--border-color)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#16213e', borderBottom: '1px solid var(--border-color)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#ccc' }}>bash — {ptsName || 'connecting...'}</span>
          </div>
        ...
        {loading ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>Connecting to terminal...</div>
        ) : error && !ptsName ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--danger)' }}>{error}</div>
        ) : (
          <>
            <div ref={outputRef} style={{ flex: 1, overflowY: 'auto', padding: 8, fontFamily: "'JetBrains Mono', monospace", fontSize: 12, lineHeight: 1.4, color: '#ccc', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }} onClick={() => inputRef.current?.focus()}>
              <div dangerouslySetInnerHTML={{ __html: output ? formatTerminalOutput(output) : '<span style="color:var(--accent-primary)">Connected. Type a command...</span>' }} />
            </div>
            <div style={{ display: 'flex', gap: 4, padding: 8, borderTop: '1px solid var(--border-color)', background: '#16213e' }}>
              <input ref={inputRef} value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyDown={handleKeyDown} disabled={!connected} style={{ flex: 1, padding: '4px 8px', background: '#0a0a1a', border: '1px solid var(--border-color)', borderRadius: 4, color: '#ccc', fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }} placeholder="Type a command..." />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 8px', background: '#16213e', borderTop: '1px solid var(--border-color)' }}>
              <span style={{ fontSize: 9, color: connected ? 'var(--success)' : 'var(--danger)' }}>{connected ? '\u25cf Connected' : '\u25cb Disconnected'}</span>
              <span style={{ fontSize: 9, color: 'var(--text-muted)' }}>Ctrl+C interrupt · ↑↓ history · Esc close</span>
            </div>
          </>
        )}
      </div>
