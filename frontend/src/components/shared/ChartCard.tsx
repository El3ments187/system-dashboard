...
function computeStats(history: MetricHistoryPoint[] | null) {
  const valid = history?.filter(p => p.value != null && !isNaN(p.value as number)).map(p => p.value as number) ?? [];
...
export default function ChartCard({ genTpsHistory, promptTpsHistory }: ChartCardProps) {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      if (r.width > 1 && r.height > 1) setChartSize({ width: Math.floor(r.width), height: Math.floor(r.height) });
    };
  ...
  const chartData = useMemo(() => {
    ...
    const maxLen = Math.max(genTpsHistory?.length ?? 0, promptTpsHistory?.length ?? 0);
    return Array.from({ length: maxLen }, (_, i) => {
      const g = genTpsHistory?.[i];
      const p = promptTpsHistory?.[i];
      return {
        x: i,
        timestampMs: g?.timestamp instanceof Date ? g.timestamp.getTime() : Date.now(),
        timeLabel: g ? formatTime(g.timestamp) : '',
        genTps: g?.value != null ? Math.round((g.value as number) * 10) / 10 : null,
        promptTps: p?.value != null ? Math.round((p.value as number) * 10) / 10 : null,
      };
    });
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <MetricCard label="Gen TPS" value={genStats.current} color="var(--success)" />
           ...
           <MetricCard label="Prompt TPS" value={promptStats.current} color="var(--accent-primary)" />
     ...
     <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
       ...
       <div ref={chartRef} style={{ height: 180, overflow: 'hidden' }}>
         <div style={{ width: chartSize.width, height: chartSize.height }}>
           <AreaChart data={chartData} width={chartSize.width} height={chartSize.height} margin={{ top: 2, right: 0, left: 0, bottom: 0 }} padding={{ top: 2, right: 2, left: 0, bottom: 0 }}>
             <CartesianGrid stroke="var(--chart-grid)" strokeDasharray="3 3" />
             <XAxis dataKey="x" type="number" domain={[0, dataMaxX]} tick={{ fontSize: 8, fill: 'var(--text-muted)' }} axisLine={{ stroke: 'var(--chart-axis)' }} tickFormatter={(v: number) => { const pt = chartData[Math.round(v)]; return pt ? pt.timeLabel : ''; }} interval="equidistantPreserveStart" />
             <YAxis width={24} type="number" domain={[0, 'dataMax']} tick={{ fontSize: 8, fill: 'var(--text-muted)' }} axisLine={{ stroke: 'var(--chart-axis)' }} tickFormatter={(v: number) => `${v} TPS`} />
             <Tooltip isAnimationActive={false} content={(props: any) => {
               ...
               return (
                 <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 4, padding: '4px 8px', fontSize: 9 }}>
                   ...
                   <div style={{ color: 'var(--success)' }}>Gen TPS: {d.genTps ?? '\u2014'}</div>
                   <div style={{ color: 'var(--accent-primary)' }}>Prompt TPS: {d.promptTps ?? '\u2014'}</div>
                 </div>
               );
