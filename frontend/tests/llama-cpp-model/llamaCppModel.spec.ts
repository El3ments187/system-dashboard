...
async function goToLlamaCpp(page: import('@playwright/test').Page): Promise<void> {
  await page.addInitScript(() => {
    localStorage.setItem('llama_cpp_dir', '/home/gamer/Documents/AI/llama.cpp/git/llama.cpp');
    localStorage.setItem('llama_scan_dir', '/home/gamer/Documents/AI/Start_Scripts');
  ...
  await expect(page.getByText('ACTIVE MODEL').first()).toBeVisible({ timeout: 15000 });
  await metricsResponse;
  await page.waitForTimeout(300);

  await expect(page.locator('text=/b\\d{4,5}/').first()).toBeVisible({ timeout: 20000 });
  await expect(page.locator('text=/Running|Stopped/').first()).toBeVisible({ timeout: 20000 });
}
...
async function generate(prompt: string, maxTokens = 200): Promise<void> {
  ...
  if (!res.ok) throw new Error(`llama.cpp /v1/chat/completions returned ${res.status}`);
...
test.describe('LlamaCppModel - steady state', () => {
  ...
  test('gen-status-badge is visible and shows a known state', async ({ page }) => {
  ...
  test('Context Max tile contains a numeric token count', async ({ page }) => {
    await expect(page.locator('[data-testid="ctx-max"]')).toContainText(/[\d,]+/);
  });
  ...
  test('console has log lines from server startup', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="console-empty-state"]')).not.toBeVisible({ timeout: 10000 });
  });
  ...
  test('Prompt Speed banner is visible', async ({ page }) => {
    await expect(page.locator('[data-testid="thrpt-prompt-tps"]')).toBeVisible();
  });
  ...
  test('console-status shows Live when model is connected', async ({ page }) => {
    await expect(page.locator('[data-testid="console-status"]')).toContainText(/Live/);
  });

  test('jump-to-latest button appears and dismisses when log area has content', async ({ page }) => {
...
test.describe('LlamaCppModel - live generation', () => {
  ...
  test('gen badge shows Generating during active completion', async ({ page }) => {
    ...
    try {
      await expect(page.locator('[data-testid="gen-status-badge"]'))
        .toHaveText('Generating', { timeout: 20000 });
    } finally {
      await genPromise;
    }
  });

  test('Context Current tile updates to a non-zero value during generation', async ({ page }) => {
    const genPromise = generate(
    ...
    try {
      ...
      await expect(page.locator('[data-testid="ctx-current"]')).not.toContainText('—', { timeout: 20000 });
    } finally {
      await genPromise;
    }
  });

  test('Gen Speed tile shows a t/s value after completion', async ({ page }) => {
    await generate('What is 7 times 8?', 50);
    // After generation ends the last gen TPS value should be visible.
    await expect(page.locator('[data-testid="thrpt-gen-tps"]')).toContainText(/\d/, { timeout: 10000 });
  });

  test('Prompt Speed tile shows a t/s value after completion', async ({ page }) => {
    await generate('What is the capital of France?', 50);
    await expect(page.locator('[data-testid="thrpt-prompt-tps"]')).toContainText(/\d/, { timeout: 10000 });
  });

  test('Generated tile increments after a completion', async ({ page }) => {
    const tile = page.locator('[data-testid="thrpt-generated"]');
    const before = parseInt((await tile.innerText()).replace(/\D/g, '') || '0', 10);
...
test.describe('LlamaCppModel - cache hits', () => {
  ...
  test('Cache Hits tile increments after a repeated prompt', async ({ page }) => {
    const cacheTile = page.locator('[data-testid="ctx-cache-hits"]');
    const before = parseInt((await cacheTile.innerText()).replace(/\D/g, '') || '0', 10);
