...
export default async function globalSetup(): Promise<void> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (res.ok) {
        const json = await res.json() as { data?: { llama_server?: { available?: boolean; model_path?: string } } };
  ...
  throw new Error(`[globalSetup] Timed out after ${TIMEOUT_MS}ms waiting for llama.cpp model`);
}
