...
export async function expectNoBlackElements(page: Page) {
  const suspicious = await page.evaluate(() => {
    ...
    for (const el of Array.from(all)) {
      ...
      if (bg === 'rgb(0, 0, 0)' && rect.width > 200 && rect.height > 80) return true;
    }
    return false;
  });
...
export async function expectAccentModeSet(page: Page, mode: string) {
  const current = await page.evaluate(() => document.documentElement.getAttribute('data-accent-mode'));
...
export async function getPerCoreColors(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    const els = document.querySelectorAll('[data-testid="per-core-bar"], [data-testid="per-core-legend-swatch"]');
    return Array.from(els)
      .map(el => (el.getAttribute('data-core-assigned-color') || el.getAttribute('data-core-color') || '').toLowerCase())
      .filter(Boolean);
  });
}

export async function expectPerCoreUniqueColorCount(page: Page, minimum: number) {
  const colors = await getPerCoreColors(page);
  const unique = new Set(colors).size;
  expect(unique, `expected >= ${minimum} unique per-core colors, got ${unique} from ${colors.length} elements`).toBeGreaterThanOrEqual(minimum);
}
