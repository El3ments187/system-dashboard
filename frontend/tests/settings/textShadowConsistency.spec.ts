  ...
  for (const mode of MODES) {
    test(`${mode} - matching UI elements have consistent text shadows`, async ({ page }) => {
      ...
      if (await settingsTitles.count() > 0) {
        const titleShadow = await settingsTitles.evaluate(el => getComputedStyle(el).textShadow);
      ...
      if (await fieldLabels.count() > 0) {
        const labelShadow = await fieldLabels.evaluate(el => getComputedStyle(el).textShadow);
      ...
      const titleShadows = await page.$$eval('.settings-card-title', els => els.map(el => getComputedStyle(el).textShadow));
      ...
      const labelShadows = await page.$$eval('.settings-field-label', els => els.map(el => getComputedStyle(el).textShadow));
      ...
      const inputShadows = await page.$$eval('.settings-input', els => els.map(el => getComputedStyle(el).textShadow));
      ...
      const buttonShadows = await page.$$eval('.settings-btn', els => els.map(el => getComputedStyle(el).textShadow));
