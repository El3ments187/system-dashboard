...
async function getStorageChartStrokes(page: Page) {
  return page.evaluate(() => {
    const card = Array.from(document.querySelectorAll('*')).find(el => el.textContent?.trim() === 'STORAGE PERFORMANCE');
    const container = card?.closest('.metric-card') || document;
    const paths = Array.from(container.querySelectorAll<SVGPathElement>('path[stroke]'));
    return paths
...
test.describe('Storage Performance chart - accent participation in Solid mode', () => {
  ...
  test('device colors change when the selected accent changes', async ({ page }) => {
  ...
  test('device colors are not pinned to the fixed per-core palette (blue/teal) when accent is orange', async ({ page }) => {
    await setAccent(page, 'orange');
    await page.waitForTimeout(150);
...
test.describe('Storage Performance chart - read/write dual-line relationship', () => {
   ...
   */
  test('each device has a solid read line and a dashed write line in a related (not identical) color', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      const heading = Array.from(document.querySelectorAll('*')).find(el => el.textContent?.trim().toLowerCase() === 'storage performance');
      const container = heading?.closest('.metric-card') || document;
      return container.querySelectorAll('.recharts-area-curve[stroke-dasharray="6 4"]').length > 0;
    }, { timeout: 10000 });

    const pairs = await page.evaluate(() => {
      const heading = Array.from(document.querySelectorAll('*')).find(el => el.textContent?.trim().toLowerCase() === 'storage performance');
      const container = heading?.closest('.metric-card') || document;
      const paths = Array.from(container.querySelectorAll<SVGPathElement>('.recharts-area-curve'));
      return paths
        .map(p => ({ stroke: p.getAttribute('stroke'), dash: p.getAttribute('stroke-dasharray') }))
        .filter(p => p.stroke && p.stroke !== 'none');
    });

    // Solid (read) and dashed (write) lines should come in matched pairs, one per device.
    const solid = pairs.filter(p => !p.dash || p.dash === '0').map(p => p.stroke!.toLowerCase());
    const dashed = pairs.filter(p => p.dash && p.dash !== '0').map(p => p.stroke!.toLowerCase());
  ...
  test('write line dash pattern matches the dashboard-wide secondary line convention', async ({ page }) => {
    const dashPatterns = await page.evaluate(() => {
      const heading = Array.from(document.querySelectorAll('*')).find(el => el.textContent?.trim().toLowerCase() === 'storage performance');
      const container = heading?.closest('.metric-card') || document;
      const paths = Array.from(container.querySelectorAll<SVGPathElement>('.recharts-area-curve[stroke-dasharray]'));
      return paths.map(p => p.getAttribute('stroke-dasharray')).filter((d): d is string => !!d && d !== '0');
    });
