...
async function goToLlamaCpp(page: import('@playwright/test').Page) {
  ...
  await expect(page.getByText('LLAMA.CPP').first()).toBeVisible({ timeout: 10000 });
}
...
test.describe('LlamaCpp - QUANT column', () => {
  test('Run Models table shows at least one non-em-dash quant cell from real profiles', async ({ page }) => {
    await goToLlamaCpp(page);
    await expect(page.getByText('Run Models').first()).toBeVisible({ timeout: 8000 });
    ...
    const quantCell = page.locator('text=/Q[0-9]_K_[MSXL]+|IQ[0-9]_[A-Z]+|BF16|F16/').first();
    await expect(quantCell).toBeVisible({ timeout: 5000 });
  });

  test('Q3_K_M is shown for the Qwen3.6-35B-A3B-REAP-RangerX profile', async ({ page }) => {
    await goToLlamaCpp(page);
    await expect(page.getByText('Run Models').first()).toBeVisible({ timeout: 8000 });

    // Qwen3.6-35B-A3B-UD-Q3_K_M-REAP-RangerX.gguf → Q3_K_M (not qat, UD, or REAP)
    await expect(page.getByText('Q3_K_M').first()).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('LlamaCpp - wrap layout', () => {
  test('hero card and model table both remain visible after toggling Wrap', async ({ page }) => {
    await goToLlamaCpp(page);
...
test.describe('LlamaCpp - console hide-idle', () => {
  test('Hide Idle toggle does not crash the page and retains page title', async ({ page }) => {
    await goToLlamaCpp(page);
  ...
  test('Hide Idle button title attribute toggles on click', async ({ page }) => {
    await goToLlamaCpp(page);
