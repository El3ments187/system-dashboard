...
test.describe('AI Page - Text Shadow Consistency', () => {
  ...
  for (const mode of MODES) {
    test(`${mode} - matching UI elements have consistent text shadows`, async ({ page }) => {
      ...
      const titleShadow = await cardTitles.evaluate(el => getComputedStyle(el).textShadow);
      ...
      if (await metricLabels.count() > 0) {
        const labelShadow = await metricLabels.evaluate(el => getComputedStyle(el).textShadow);
      ...
      if (await metricValues.count() > 0) {
        const valueShadow = await metricValues.evaluate(el => getComputedStyle(el).textShadow);
      ...
      if (await sectionTitles.count() > 0) {
        const secTitleShadow = await sectionTitles.evaluate(el => getComputedStyle(el).textShadow);
        expect(secTitleShadow).not.toBe('');
      }

      // Verify all card titles share the same text-shadow value
      const titleShadows = await page.$$eval('.card-title', els => els.map(el => getComputedStyle(el).textShadow));
      ...
      const labelShadows = await page.$$eval('.metric-tile-label', els => els.map(el => getComputedStyle(el).textShadow));
      ...
      const valueShadows = await page.$$eval('.metric-tile-value', els => els.map(el => getComputedStyle(el).textShadow));
      ...
      const buttons = page.locator('button').filter({ hasText: /Test|Open|View|Run/ }).first();
      if (await buttons.count() > 0) {
        const btnShadow = await buttons.evaluate(el => getComputedStyle(el).textShadow);
      ...
      if (await inputs.count() > 0) {
        const inputShadow = await inputs.evaluate(el => getComputedStyle(el).textShadow);
  ...
  test('All accent modes resolve to valid text-shadow values', async ({ page }) => {
    await page.goto(`${BASE_URL}${AI_PATH}`);
    // Wait for specific selectors instead of networkidle to avoid hanging on WebSocket/long-polling connections
    await page.waitForSelector('[class*="card-header"]', { timeout: 10000 });
