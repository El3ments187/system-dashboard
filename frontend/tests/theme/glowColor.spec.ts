...
async function enableNeonGlow(page: Page) {
  ...
  const isActive = await glowRow.evaluate((el) => el.classList.contains("active"));
  if (!isActive) await glowRow.click();
  await page.waitForTimeout(100);
}

async function selectGlowColorMode(page: Page, mode: "accent" | "match" | "custom") {
...
test.describe("Glow Color: Custom swatch selector", () => {
  ...
  test("Custom mode shows exactly 32 accent swatches, no free-form color input", async ({
    ...
    const swatches = page.locator('[data-testid="glow-custom-swatches"] button');
  ...
  test("Clicking Cyan swatch sets --accent-glow to a cyan color everywhere", async ({
    ...
    const cyanBtn = page.getByRole("button", { name: "Cyan" });
    await cyanBtn.click();
    ...
    const glowCustom = await page.evaluate(() =>
      document
        .documentElement
        .style.getPropertyValue("--glow-custom")
        .trim()
        .toLowerCase(),
    );
    ...
    const accentGlow = await page.evaluate(() =>
      window.getComputedStyle(document.documentElement).getPropertyValue("--accent-glow").trim(),
    );
  ...
  test("Clicking Cyan then checking the preview resolves to cyan hue", async ({ page }) => {
    await selectGlowColorMode(page, "custom");
    const cyanBtn = page.getByRole("button", { name: "Cyan" });
    await cyanBtn.click();
    await page.waitForTimeout(200);
    ...
    const isCustom = await page.evaluate(() => {
      ...
      const glowCustom = root.style.getPropertyValue("--glow-custom").trim().toLowerCase();
  ...
  test("Switching back to Match hides the swatch grid", async ({ page }) => {
    await selectGlowColorMode(page, "custom");
    await expect(page.locator('[data-testid="glow-custom-swatches"]')).toBeVisible();

    await selectGlowColorMode(page, "match");
    await expect(page.locator('[data-testid="glow-custom-swatches"]')).not.toBeVisible();
  });
  ...
  test("Picked swatch persists across reload", async ({ page }) => {
    ...
    const glowCustom = await page.evaluate(() =>
      document.documentElement.style.getPropertyValue("--glow-custom").trim().toLowerCase(),
    );
    expect(glowCustom).toBe("#06b6d4");
    const attr = await page.evaluate(() =>
      document.documentElement.getAttribute("data-glow-color"),
    ...
    const cyanBtnAfter = page.getByRole("button", { name: "Cyan" });
    await expect(cyanBtnAfter).toBeVisible();
  });

  test("Match mode: glow color attribute absent from <html>", async ({ page }) => {
    await selectGlowColorMode(page, "match");
    const attr = await page.evaluate(() =>
      document.documentElement.getAttribute("data-glow-color"),
    );
  ...
  test("Out-of-palette saved value migrates to a valid accent on load", async ({ page }) => {
