...
test.describe('LlamaCppPage - Active Model card', () => {
  ...
  test('status indicator shows running or offline', async ({ page }) => {
    await expect(page.locator('text=/Running|Offline|Loading/i').first()).toBeVisible();
  });
});
...
test.describe('LlamaCppPage - Runtime card', () => {
  ...
  test('Uptime row is visible with a time value', async ({ page }) => {
    ...
    await expect(page.locator('text=/\\d+h \\d+m|\\d+m \\d+s|\\d+s/').first()).toBeVisible();
  });
...
test.describe('LlamaCppPage - llama.cpp card', () => {
  ...
  test('Update button is visible', async ({ page }) => {
    ...
    await expect(page.getByText('Update', { exact: true }).first()).toBeVisible({ timeout: 20000 });
  });
  ...
  test('Readme button is visible', async ({ page }) => {
    await expect(page.getByText('Readme', { exact: true }).first()).toBeVisible();
  });
...
test.describe('LlamaCppPage - Run Models table', () => {
  ...
  test('at least one model row is visible', async ({ page }) => {
    await expect(page.locator('text=/Running|Stopped/').first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test('stopped model has a Run button', async ({ page }) => {
    await expect(page.locator('button').filter({ hasText: 'Run' }).first()).toBeVisible({ timeout: 10000 });
  });

  test('quant cells show known quant tokens from real profiles', async ({ page }) => {
    await expect(
...
test.describe('LlamaCppPage - Console', () => {
  ...
  test('filter buttons INFO/WARN/ERROR/DEBUG/STATS are all visible', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'WARN' }).first()).toBeVisible();
...
test.describe('LlamaCppPage - data binding sanity', () => {
  ...
  test('max context 131,072 from API matches context card MAX tile', async ({ page }) => {
