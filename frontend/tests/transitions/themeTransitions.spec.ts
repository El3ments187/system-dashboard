...
for (const { name, path } of PAGES) {
  test.describe(`${name} Page - Theme Transitions`, () => {
    ...
    for (const [from, to] of TRANSITIONS) {
      test(`${from} -> ${to}: no stale state, no black elements`, async ({ page }) => {
    ...
    test('rapid cycling through every mode leaves the dashboard in a clean final state', async ({ page }) => {
      const modes = ['solid', 'animated-gradient', 'rainbow-wave', 'spectrum', 'solid'];
    ...
    test('returning to Solid mode resolves --accent-primary back to a concrete color (not a leftover gradient/hsl expression)', async ({ page }) => {
