...
import { setAccentMode, setAccent, getPerCoreColors, expectPerCoreUniqueColorCount } from '../helpers/e2eThemeAssertions';
...
test.describe('CPU Page - Per-Core Solid Mode Exemption', () => {
  ...
  for (const accent of ['turquoise', 'red', 'purple', 'green']) {
    test(`Solid + ${accent} accent: per-core chart remains multi-colored and uses the full palette`, async ({ page }) => {
  ...
  test('per-core palette is identical across different accents in Solid mode', async ({ page }) => {
  ...
  test('per-core chart does not shift its base hue to track the selected accent', async ({ page }) => {
    ...
    await page.waitForTimeout(100);
    const accentPrimary = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--accent-primary').trim().toLowerCase());
    const colors = await getPerCoreColors(page);
    expect(colors[0]).not.toBe(accentPrimary);
  });

  test('per-core chart never collapses to a single solid color', async ({ page }) => {
...
test.describe('CPU Page - Per-Core Participates Outside Solid Mode', () => {
  ...
  for (const mode of ['animated-gradient']) {
    test(`${mode}: per-core chart participates in the active theme (accent changes shift the palette)`, async ({ page }) => {
  ...
  test('rainbow-wave: per-core chart tracks the live --accent-spin CSS variable over time', async ({ page }) => {
    ...
    await page.evaluate(() => document.documentElement.style.setProperty('--accent-spin', '190'));
    await page.waitForTimeout(900);
  ...
  test('spectrum: per-core chart stays multi-colored and ignores the selected accent', async ({ page }) => {
