  ...
  test.describe(`${name} screenshots`, () => {
    ...
    for (const mode of MODES) {
      test(`${name} - ${mode}`, async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot(`${name}-${mode}.png`, { fullPage: true });
      });
    }
  });
}
...
test.describe('dual-line chart screenshots', () => {
  ...
  for (const mode of MODES) {
    test(`memory chart (primary solid + secondary dashed) - ${mode}`, async ({ page }) => {
      ...
      await page.waitForTimeout(150);
      const chart = page.locator('text=MEMORY UTILIZATION HISTORY').locator('..').locator('..');
...
test.describe('cpu per-core screenshots', () => {
  ...
  for (const mode of MODES) {
    test(`per-core chart - ${mode}`, async ({ page }) => {
      ...
      await page.waitForTimeout(150);
      const chart = page.locator('text=PER-CORE UTILIZATION').locator('..').locator('..');
